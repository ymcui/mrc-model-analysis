...
class InputFeatures(object):
  ...
  def __init__(self,
    ...
    self.att_mask = att_mask # Note: 1 for masking, 0 for non-masking; this is different from input_mask.
    self.start_position = start_position
    self.end_position = end_position
...
SPIECE_UNDERLINE = '▁'
#
def read_squad_examples(input_file, is_training, repeat_limit=3):
  ...
  def _tokenize_chinese_chars(text):
    ...
    for char in text:
      cp = ord(char)
      if _is_chinese_char(cp) or is_spec_char(char):
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
  ...
  for (example_index, example) in enumerate(examples):
    ...
    for (doc_span_index, doc_span) in enumerate(doc_spans):
      ...
      assert len(att_mask) == max_seq_length
...
#
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, input_span_mask, att_mask,
...
def input_fn_builder(input_file, seq_length, is_training, drop_remainder):
  ...
  name_to_features = {
      ...
      "att_mask": tf.FixedLenFeature([seq_length], tf.int64),
  }
...
class FeatureWriter(object):
  ...
  def process_feature(self, feature):
    ...
    features["att_mask"] = create_int_feature(feature.att_mask)
